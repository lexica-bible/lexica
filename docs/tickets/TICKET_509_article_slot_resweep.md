...
**§6k — BUILD-LINE RECONCILIATION, 2026-08-01 (the 8/1 ride's halt walked down; pin
re-declared from evidence, the build unchanged).** The real build printed `923 written,
411 refusals (unattested 388, article 12, star 11)` against the pinned 929/332. Fully
attributed, member-level, by `audit_article_slot_carrier.py --plan --corrected` (new,
read-only, run on PA — mirrors the build's Rahlfs/TAGNT pronoun corrections + lexicon +
BH inputs and reproduced the line to the digit):
- **Counting basis:** the plan counted per carrier slot; the build counts per decision
  (a slot writing one side and refusing the other = 1 + 1 there). Uncorrected per-entry
  is 929/404 (unattested 381) — 72 of the 79 "extra" refusals were basis, not behavior.
  `--plan` now prints both bases so this cannot recur.
- **Layer:** the sizing measured the pass on RAW source; the build corrects mistagged
  pronoun numbers first. Six writes flipped to 'unattested' — all G1473 possessives
  whose neighbour number the corrections changed (2Sa 6:12 'his', Est 8:13 'their',
  Gen 24:2 'his', Gen 31:18 'his', Gen 47:1 'their', Job 13:14 'my'). Refusing on the
  truer number is the predicate working, not failing.
- **Two new entries:** one Rom 8:26 carrier the corrections mint that raw text never
  shows — moved text EMPTY, both neighbours refused, nothing written.
**RE-PIN (ruling under the standing delegation): build line = 923 written / 411
refusals (388/12/11). The 6 flipped rows stay unrepaired in the built copy, so the
live window below shifts +6 → 1,525–1,765** (baseline 2,670 unchanged). Full corrected
decision record: `plan_corrected.txt` on PA (regenerable from the committed audit).

**§6l — THE WINDOW IS SUPERSEDED (2026-08-01, ride close; ruled at the gate).** The
built copy sized **2,526** by the sizing-SQL list / **2,520** by the residue predicate
(bridge members named by `--predict-vs`) against baseline **2,670** — far above the
1,525–1,765 window, and the member evidence says the WINDOW was wrong, not the build:
- New entrants **0**. Departures **144** (grouped: 'and' 87, 'but' 22, 'the whole' 5,
  'both' 4, punctuation-float singles — `live_diff.txt` on PA, regenerable via
  `--live-diff`).
- The decisive member: `1Co 1:18` — live AND the copy both carry 'but' on G1161; only
  slot order differs. Live's old build had already repaired most of the 923 write
  sites through its existing folds; the pass's genuinely NEW repairs vs live are the
  144. The window's bins were measured by re-running the build BARE (no pronoun
  corrections, no lexicon, no BH rows) — a model of live that live never was.
- **The swap condition is now `audit_article_slot_carrier.py --predict-vs <copy>`:**
  the expected leftover set re-derived on the TRUE layer (corrections + real lexicon
  + BH), member equality required, plus a DECLARED 15-member tail allowance, each
  member named to its owning pinned step (fix_idios_own ×13 — exactly the
  G2398-unattested rows the pass correctly refused, repaired by the hand patch — and
  the two documented blank-"G." fills, Act 24:8 / Mat 12:14). **Result on the 8/1
  copy: SET-EQUALITY PASS.** A count is a tripwire; the member set is the ruling.
**STANDING TRAP (four casualties in one ride — flipped writes, audit refusals, the
window, the sizing bins): any figure asserted about a post-correction artifact must
be DERIVED THROUGH the correction layer with the same inputs the build uses, and the
derivation command committed with the figure.**

**EXPECTED LIVE PICTURE AFTER A FUTURE REBUILD (pre-registered per the verdict gate —
window since re-pinned +6 by §6k above, then SUPERSEDED ENTIRELY by §6l):**
