...
- **ABP-tab routing via the folded header (2026-09-06, `CHARTER_abp_tab_routing.md`).** A Greek number with NO words row (starred name slots — Galilee G1056) used to grey the ABP tab and silently fall back to KJV. `_header_bridge` now routes it through its folded header when, and only when: the lexicon lemma folded like `lemma_plain` reaches numberless rows that are ALL `source='surface'` (no `lemma-only` residual — Zion 168+1 / Aram 12+5 refused until folded), ONE stored value, and the key has ONE lexicon owner (γαζα, ηλι collide → refused). `_bridge_if_grey` consults it only when the production predicate finds nothing, so MIXED numbers (47 open today with a same-header numberless residue — own follow-up) and ordinary words never change. Bridged: the ABP branch (book counts, All-books + per-book lists with `position`, total) = `_pn_lemma_rows(header)` — the PN: page's derivation, PARITY locked in `tests/test_abp_tab_bridge.py`; renderings stay empty; profile carries `abp_header` (JP-approved field, present only when bridged) which drives the ruled state line under the count. Israel / Jerusalem (lemma-only only, one value) HELD by ruling: they route only once the header lane vets them as `surface`, never via a lemma-only door. Census tool: `scripts/audit_abp_tab_bridge.py` (read-only, live + a pre-fold copy).
