...
- **Rail + jump-marker reconcile (2026-06-29):** the reader's real position lives in LibraryView
  ...
  `selBook`/`selChapter` without it. Also here: `setOrder` canonical keeps the passage's book.
  Memory `project_nav_reconcile`.
- **⚠ A JUMP'S `translation` IS A ONE-TIME INSTRUCTION, NOT STANDING STATE — and this rule was
  already written here and STILL got bitten a third time (2026-08-09).** The nav effect applies
  `nav.translation` every time the nav object changes, so any re-emit that spreads the old bag
  re-applies a Bible the reader has since left. Bite 1: search baked it in (`jumpToHit`). Bite 2:
  page turn (`turnPage` now hand-builds a clean object). Bite 3: the version-switch re-scroll —
  the reader clicked ABP, the re-emit carried `"kjv"`, and the nav effect put them back a beat
  later, so the tab click *fired and lost a race*. `extern` rides along the same way.
  **Why the old wording failed: it named a symptom ("the page-turn text-snap bug") instead of the
  field's nature, so the next author checked whether they were turning a page — not whether they
  were re-emitting.** Enforced now, not just documented: `navAfterVersionSwitch`
  (`56-library-order-logic.jsx`, gated in `tests/test_library_order.js`). **Two producers set it,
  and a fix must cover BOTH:** Word study's verse list (`90-app.jsx:476`) and the SEO reader
  deep-link `/?b=&c=&t=` (`:296-302`, generated by `views_seo.py:_reader_link` → the
  "Open in the interactive reader" link on `/read/<book>/<ch>/<text>` pages). Check for a third
  before assuming two. The queued cure is to CONSUME the value once applied so no re-emit can
  re-assert it — own session; it touches the load-bearing nav effect. **`onReaderPos`/`handleReaderPos` carry only (book, chapter) — NO
