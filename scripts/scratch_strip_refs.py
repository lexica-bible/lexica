...
def main():
    ...
    ap.add_argument("--strip", action="append", required=True, metavar="REF",
    ...
    after_raw = strip_refs(before_raw, args.strip)
    after_sb = strip_refs(before_sb, args.strip)
    hit = (after_raw != before_raw) or (after_sb != before_sb)

    print(f"{sid}: stripping {args.strip}  — {'found + would change' if hit else 'NOT FOUND (no change)'}")
    if not hit:
