...
def _exempt_static():
    ...
    return ep == "static" or ep.startswith("seo.") or ep in {"favicon", "apple_touch_icon", "robots_txt", "app_bundle"}
...
def _asset_helpers():
    ...
    def bundle_url():
        # app.js is served by the app_bundle route (long cache header) instead of the
        # /static/ mapping. Keep the same ?v=<mtime> cache-bust so a deploy changes the
        # URL and the long cache never serves a stale bundle.
        try:
            mtime = int(os.path.getmtime(os.path.join(app.static_folder, "app.js")))
        except OSError:
            mtime = 0
        return url_for("app_bundle", v=mtime)
    return {"asset_url": asset_url, "bundle_url": bundle_url}
...
# The app bundle is served HERE (not via PythonAnywhere's /static/ mapping) so it can
# carry a long Cache-Control. PA's static server has no knob for cache headers, so on
# /static/ the 543KB bundle ships with none and browsers that don't heuristically cache
# it (Firefox) re-fetch the whole file every load — the app paints ~1s late. The URL
# still carries ?v=<mtime> (see bundle_url), so a deploy changes the URL → fresh fetch;
# that makes the year-long cache safe — it can never serve a stale bundle.
@app.route("/assets/app.js")
def app_bundle():
    resp = send_from_directory(app.static_folder, "app.js")
    resp.headers["Cache-Control"] = "public, max-age=31536000, immutable"
    return resp


# ── TEMPORARY streaming/buffering probe (Step 0) — REMOVE after the test ──────
# Emits 10 pieces 0.5s apart, each stamped with the server emit time, in the same
# SSE shape the Ask-corpus synthesis stream would use. A client (curl -N) can then
# tell trickle (timestamps tick ~0.5s apart) from buffered (all 10 land at once at
# the end). X-Accel-Buffering:no asks nginx not to hold the response. This answers
# whether streaming the synthesis is even possible through PythonAnywhere + Cloudflare.
@app.route("/api/_streamtest")
def _stream_test():
    import time
    ...
    def gen():
        start = time.time()
        for i in range(10):
            yield f"data: chunk {i} (server t+{time.time() - start:0.2f}s)\n\n"
            time.sleep(0.5)
        yield "data: done\n\n"
