      ...
      if (w.italic_words && w.english && w.english.includes(' ')) {
        return (
          <span key={key} data-note-pos={w.position}
            ...
            onClick={clickable ? () => onWordClick(isPN ? { ...makeEntry(w), ...pnClickPayload(w) } : makeEntry(w)) : undefined}>
      ...
      content = groups.map((g, gi) => {
        ...
        for (const w of g.words) {
          ...
          if (m && m.index > 0) {                       // keep at least one real word before the mark
            bracketTrail += m[0].trim();
            cleaned.push({ ...w, english: eng.slice(0, m.index).trimEnd() });
          } else {
            cleaned.push(w);
          }
        }
        if (bracketTrail) {
          bracketTrail = /^(?:--|—|–)+$/.test(bracketTrail) ? " " + bracketTrail : bracketTrail;
        }
        const ordered = orderBracketGroupWords(cleaned);
