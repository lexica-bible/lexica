...
function GraphSvg({ claims, overlay, verdict, shared, onNavigate }) {
  ...
  Object.keys(segSucc).forEach(a => (segSucc[a] || []).forEach(b => {
    if (a in adj && b in adj) { adj[a].push(b); adj[b].push(a); }
  }));
  const compOf = {}; let nComp = 0;
  nodes.forEach(id => {
    if (id in compOf) return;
    const stack = [id]; compOf[id] = nComp;
    while (stack.length) {
      const x = stack.pop();
      (adj[x] || []).forEach(n => { if (!(n in compOf)) { compOf[n] = nComp; stack.push(n); } });
    }
    nComp++;
  });
  if (nComp > 1) {
    ...
    for (let g = 0; g < nComp; g++) {
      ...
      comps.push({ mem, lo: Math.min(...mem.map(id => yy[id] || 0)), hi: Math.max(...mem.map(id => yy[id] || 0)) });
    }
    comps.sort((a, b) => a.lo - b.lo);          // keep the clusters' existing top-to-bottom order
    let cursor = 0;
    comps.forEach(cm => {
      const shift = cursor - cm.lo;
      cm.mem.forEach(id => { yy[id] = (yy[id] || 0) + shift; });
