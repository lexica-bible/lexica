...
function useSwipeToDismiss(onClose) {
  ...
  React.useEffect(() => {
    ...
    if (!el) return;
    let startY = 0, dragY = 0, active = false, fromChrome = false, box = null;
    const SNAP = 'transform 0.25s cubic-bezier(0.2,0.8,0.2,1)';

    // WHICH element is actually scrolling under this touch.
    ...
    const scrollBoxAt = (target) => {
      ...
      return null;                                  // genuinely nothing scrolling: chrome-only sheet
    };
    const atTop = () => !box || box.scrollTop <= 0;
    const onStart = (e) => {
      box = scrollBoxAt(e.target);                  // resolve per gesture, not once at mount
