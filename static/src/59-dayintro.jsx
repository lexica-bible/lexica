...
function TimelineStrip({ win }) {
  ...
  const a = Math.min(frac(y0), frac(y1)), b = Math.max(frac(y0), frac(y1));
  ...
  return (
    <div className="dintro-tl">
      <div className="dintro-tl-track">
        <div className="dintro-tl-now" style={{ left: at(a), width: `calc(${b - a} * (100% - 7px))` }} />
        {et.marks.map((m, i) => (
          <div key={i} className="dintro-tl-dot" style={{ left: at(frac(m.year)) }} aria-hidden="true"></div>
        ))}
      </div>
