...
function LibraryView({ nav, onNavChange, onReaderPos, onWordClick, onVerseNumberClick, onOpenNote, onTranslationChange, isMobile, showSummary, focusMode, onToggleFocus, onDetailBaseChange, libFontSize, changeFontSize, theme, setTheme }) {
  ...
  // Mode three (faithful ABP interlinear): Greek-dominant, own render branch. ABP
  // Bible ONLY — guarded on translation==="abp" so KJV/BSB/Hebrew/non-canon never
  // enter it (a shared viewMode="interlinear" must not misroute another text).
  const abpMode     = translation === "abp" && !nonCanon && viewMode === "interlinear";
  // Non-ABP texts have no mode three, so they read "interlinear" as plain chip.
  const chipMode    = !proseLocked && !abpMode && (viewMode === "chip" || viewMode === "interlinear" || showStrongs || showInterlinear);
  ...
  // 3-way reading control (Chip · Interlinear · Prose). Interlinear (mode three) is
  // ABP-only; on other texts the button is hidden and the control stays 2-way.
  const ilApplicable  = translation === "abp" && !nonCanon;
      ...
      {navVisible ? (
        <div className="lib-bar">
          <div className="lib-bar-l">
            ...
            <span className="lib-bar-sep" aria-hidden="true"/>
            <button className={"lib-toggle lib-toggle-icon" + (showStrongs ? " on" : "")} disabled={proseLocked} title="Strong's numbers" aria-label="Strong's numbers" aria-pressed={showStrongs} style={proseLocked ? { opacity: 0.35, cursor: "default" } : undefined} onClick={() => !proseLocked && toggleStudy("showStrongs")}><Icon.Hash/></button>
            <button className={"lib-toggle lib-toggle-icon" + (showInterlinear ? " on" : "")} disabled={proseLocked} title="Interlinear" aria-label="Interlinear" aria-pressed={showInterlinear} style={proseLocked ? { opacity: 0.35, cursor: "default" } : undefined} onClick={() => !proseLocked && toggleStudy("showInterlinear")}><Icon.Interlinear/></button>
            {!nonCanon && (
