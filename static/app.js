...
function GraphSvg({
  ...
  const wpOf = {},
    dcol = {};
  const segPre = {},
    segSucc = {}; // chain-segment links that drive the ordering
  const seg = (a, b) => {
    (segPre[b] = segPre[b] || []).push(a);
    (segSucc[a] = segSucc[a] || []).push(b);
  };
  carry.forEach(l => {
    const wps = [];
  ...
  const byCol = {};
  nodes.forEach(id => {
    (byCol[colOf(id)] = byCol[colOf(id)] || []).push(id);
  });
  const rank = id => {
  ...
  cols.forEach(c => {
    if (c > 0) relax(c, segPre);
  }); // seed everyone with one forward pass
...
function PanZoom({
  ...
  const fit = () => {
    ...
    tf.current = {
      s,
      x: Math.max(0, (v.clientWidth - natW * s) / 2),
      y: 6
    };
    apply();
  };
  ...
  const onWheel = e => {
    const [mx, my] = vpt(e.clientX, e.clientY);
    zoomTo(tf.current.s * (e.deltaY < 0 ? 1.1 : 0.9), mx, my);
    e.preventDefault();
  };
  const zoomBtn = f => () => {
    const v = view.current;
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "study-pz-inner",
    ref: inner
  }, children)), /*#__PURE__*/React.createElement("div", {
    className: "study-pz-tools"
  }, /*#__PURE__*/React.createElement("button", {
    className: "study-pz-btn",
    ...
    className: "study-pz-btn study-pz-btn--fit",
    onClick: fit,
...
function GraphChart({
  ...
  const guardedNav = onNavigate ? (b, c, v) => {
    if (!movedRef.current) onNavigate(b, c, v);
  } : undefined;
  ...
  }, label), /*#__PURE__*/React.createElement(PanZoom, {
    movedRef: movedRef,
    resetKey: i
...
function StudyView({
  ...
  const [initialOpen] = useState(() => {
    try {
      return localStorage.getItem("lexica.study.open.v1") || "";
    } catch (e) {
      return "";
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem("lexica.study.module.v1", module);
    } catch (e) {}
  }, [module]);
  useEffect(() => {
    try {
      if (editing && editing.id) localStorage.setItem("lexica.study.open.v1", editing.id);else localStorage.removeItem("lexica.study.open.v1");
    } catch (e) {}
  }, [editing && editing.id]);
  useEffect(() => {
