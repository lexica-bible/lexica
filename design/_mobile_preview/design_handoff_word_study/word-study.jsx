    ...
    <header className="hdr">
      <div className="hdr-inner">
        ...
        <nav className="hdr-nav">
          ...
          <a className="hdr-link" href="Study.html">Study</a>
...
// A gloss search (“spirit”) maps to several lemmas. Instead of a dead-end list, we land in
// the top lemma's full study view and show this bar to pivot between the senses.
function SensesBar({ label, senses, active, onPick }) {
  ...
  return (
    <div className={"glsenses" + (open ? " open" : "")}>
      <button className="glsenses-head" onClick={() => setOpen(!open)} aria-expanded={open}>
        <span className="glsenses-l"><b>{senses.length}</b> words rendered “{label}”</span>
        {!open && cur && (
          <span className="glsenses-cur">
            <span className={"glsenses-cur-gk" + (cur.script === "hebrew" ? " heb" : "")}>{cur.word}</span>
            <span className="glsenses-cur-tr">{cur.translit} · {cur.occ}</span>
          </span>
        )}
        <span className="glsenses-tog">{open ? "Collapse" : "Expand"} <I.Chevron/></span>
      </button>
      {open && (
          ...
          {senses.map((s) => {
            ...
            return (
              <button key={s} className={"glrow" + (s === active ? " on" : "")} onClick={() => onPick(s)}>
                ...
                <span className="glrow-occ">{l.occ} <I.ArrowSm/></span>
              </button>
            );
          })}
        </div>
...
function App() {
  ...
  const [glossSenses, setGlossSenses] = useState([]);   // lemmas that share a searched gloss
  ...
  const lex = (view === "overview") ? null : LEMMAS[activeStrong];
  ...
  const studyLemma = (s) => {
    if (!LEMMAS[s]) return;
    setActiveStrong(s); setView("lemma"); setBookFilter(null); setGlossSenses([]);
    if (vw <= 1040) setDetailOpen(true);
  };
  // pivot between senses of a gloss — keeps the senses bar in place
  const pickSense = (s) => {
    if (!LEMMAS[s]) return;
  ...
  const onSubmit = (term) => {
    ...
    // a plain-English gloss (or an ambiguous term) that maps to several lemmas → senses
    let senses = null;
    ...
    if (senses && senses.length) {
      ...
      setActiveStrong(senses[0]); setView("lemma"); setBookFilter(null);   // land in the top sense's full study view
      if (vw <= 1040) setDetailOpen(true);
    }
  };
              ...
              {lex ? (
                <>
                  {glossSenses.length > 1 && glossSenses.includes(activeStrong) && (
                    <SensesBar label={glossLabel} senses={glossSenses} active={activeStrong} onPick={pickSense}/>
                  )}
                  {bookFilter && (
